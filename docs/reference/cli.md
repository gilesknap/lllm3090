...
| `start <name> [--ctx N]` | Stop any running engine and start this model |
...
`start` takes the context window from the catalogue when the model is a known
one, and falls back to 32768 otherwise. Anything after `claude` is passed
