...
The same things the browser panel shows — engine state, VRAM, what is
installed, the catalogue with what fits and what it will do, downloads with
progress, and the tail of the engine log — drawn in the terminal.
...
| `Tab` | switch between the installed list and the catalogue |
| `s` | start the model under the cursor |
| `x` | stop the engine and free the VRAM |
| `d` | download the catalogue entry under the cursor |
| `c` | cancel that download |
| `P` | start the panel, if it is not running |
| `q` | quit — the engine keeps running |

`s` works in both panes: a catalogue entry that is already downloaded is a
model you can start.
...
```
lllm3090 0.4.1                             NVIDIA GeForce RTX 3090 24 GB
------------------------------------------------------------------------
engine   running  Qwen3.6-35B-A3B                    http://127.0.0.1:1919
VRAM     [############################......] 19.7 / 24.0 GiB
 INSTALLED --------------------------------------------------------------
 > Qwen3.6-35B-A3B              17.7 GB                          running
   Qwen3-8B                      5.0 GB
*CATALOGUE --------------------------------------------------------------
   Qwen3.6-35B-A3B           17.7G moe         212k x2  ~126 tok/s  installed
 > Muse-Glimmer-30B          17.9G dense+vis   128k x3  ~44 tok/s  [###...] 41.2%
   Gemma-4-12B-QAT            6.9G dense+vis   256k x4  ~84 tok/s   installed
 ENGINE LOG -------------------------------------------------------------
  srv  load_model: loaded 48 layers to GPU
  main: server is listening on http://127.0.0.1:1919
```
